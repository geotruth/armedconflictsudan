...
events %>% 
  ...
  mutate(actor1 = case_when(
    actor1 == "Military Forces of Sudan (2019-) Military Intelligence Service" ~ "Military Forces of Sudan (2019-)",
    TRUE ~ actor1
  ), 
  actorshort = case_when(
    actor1 %in% "Military Forces of Sudan (2019-)" ~"MSF", 
    actor1 %in% "Rapid Support Forces" ~ "RSF", 
    TRUE ~ "Other"
  )) %>% 
...
# Saving the plot with larger dimensions
# 
...
events %>%
  ...
  ungroup() %>% 
  complete(admin1Name = unique(sudan_boundaries$admin1Name), fill = list(fatalities = 0))->actor_fatalities
...
plotmaps <- function(x) {
  ...
  ggplot()+
    ...
    geom_richtext(
      data = sudanregion_label,
      aes(
        x = ifelse(admin1Name %in% x[["admin1Name"]], no = long, yes = long), 
        y = ifelse(admin1Name %in% x[["admin1Name"]], no = lat, yes = lat+1.3), 
        label = admin1Name),
      size = 25,
      na.rm = TRUE,
    ...
    geom_sf(data = x %>% filter(admin1Name %in% x[["admin1Name"]]),
            mapping = aes( alpha=.25, linewidth= .0016), fill = "#dfc27d")+  
